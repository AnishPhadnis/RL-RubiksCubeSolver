...
class FeudalNetwork(nn.Module):
    def __init__(self,
        ...
        self.preprocessor = Preprocessor(input_dim, device, mlp) # baseline of some sort?
        self.percept = Perception(input_dim[-1], self.d, mlp) 
        self.manager = Manager(self.c, self.d, input_dim, self.r, args, device)
...
class Perception(nn.Module):
    def __init__(self, input_dim, d, mlp=True): # mlp=False
        super().__init__()
        ...
        #         nn.ReLU())
        self.percept = nn.Sequential(
            nn.Linear(input_dim, 16), # 16 pieces
            nn.ReLU(),
            nn.Linear(16, d), # 6 faces
            nn.ReLU())
        # else:
        #     self.percept = nn.Sequential(
...
class Manager(nn.Module):
    def __init__(self, time_horizon, hidden_size, input_size, dilation_level, args, device):
        super().__init__()
        self.time_horizon = time_horizon   # Time horizon
        self.hidden_size = hidden_size     # Hidden size of LSTM
        self.dilation_level = dilation_level   # Dilation level
        self.eps = args.eps   # Exploration parameter
        self.device = device   # Device (cpu or gpu)

        # Linear layer for mapping state to goal
        self.space_mapping = nn.Linear(input_size[0], self.hidden_size)

        # Dilated LSTM layer
        self.dilated_lstm = DilatedLSTM(input_size[0], self.hidden_size, self.dilation_level)

        # Critic network for estimating value of goal
        self.critic = nn.Linear(self.hidden_size, 1)

    def forward(self, state, hidden, mask):
        """
        Given a state, returns a goal, value estimate and updated hidden state for the Manager.

        Args:
            state (Tensor): current state of size B x D, where B is batch size and D is hidden size.
            hidden (Tuple[Tensor, Tensor]): previous hidden state of the Manager, tuple of tensors each of size B x D.
            mask (Tensor): binary tensor of size B x 1 used for masking previous hidden state.

        Returns:
            Tuple[Tensor, Tuple[Tensor, Tensor], Tensor, Tensor]: Tuple containing:
                - goal (Tensor): goal tensor of size B x D.
                - hidden (Tuple[Tensor, Tensor]): updated hidden state of Manager.
                - state (Tensor): detached state tensor of size B x D.
                - value_estimate (Tensor): value estimate tensor of size B x 1.
        """
        # Map state to goal space
        state2 = torch.transpose(state, 0, 1)
        print(state2)
        mapped_state = self.space_mapping(state2).relu()
        print("MAPPED STATE:")
        print(mapped_state)
        print(len(mapped_state))

        # Mask hidden state before passing to LSTM
        masked_hidden = (mask * hidden[0], mask * hidden[1])

        # Pass masked hidden state and mapped state through dilated LSTM
        goal_hat, updated_hidden = self.dilated_lstm(mapped_state, masked_hidden)

        # Estimate value of goal
        value_estimate = self.critic(goal_hat)

        # Normalize goal_hat to obtain goal
        goal = normalize(goal_hat)

        # Detach state for computational efficiency
        state = state.detach()

        # With probability eps, select a random goal to encourage exploration
        if (self.eps > torch.rand(1)[0]):
            goal = torch.randn_like(goal, requires_grad=False)

        return goal, updated_hidden, state, value_estimate

    def state_goal_cosine(self, states, goals, masks):
        """
        Computes the cosine distance between the difference state s_{t+c} - s_{t}
        and the goal embedding at timestep t g_t(theta).

        Args:
            states (List[Tensor]): List of tensors of size 2C+1 x B x D, where C is time horizon,
                                   B is batch size and D is hidden size.
            goals (List[Tensor]): List of tensors of size 2C+1 x B x D.
            masks (List[Tensor]): List of tensors of size C-1 x B x 1.

        Returns:
            Tensor: cosine distance tensor of size B x 1.
        """
        t = self.time_horizon
        print(masks[t: t + self.time_horizon - 1])
        mask = torch.stack(masks[t: t + self.time_horizon - 1]).prod(dim=0)
        cosine_dist = d_cos(states[t + self.time_horizon] - states[t], goals[t])
